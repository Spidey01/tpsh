...
completed, are assumed to be filenames. Filenames given this way will be executed as a shell script, non interactively.
...
Enclosing characters within double quotes restricts the evaluation to
parameter expansion and backslash escapes. The backslash inside double quotes
is historically weird: and the shell does not yet understand them.
...
Complex commands are broken down enqueued (into) simple commands. The first
word of each simple command is checked for expansion in accordance with the
shells quoting rules, and then executed by the shell. To illustrate a point,
if the following simple command was given to tpsh:

	$ vi -c "set ai" file.c

the argument vector (argv) or vi will be as follows:
...
(inclusive) of I<first> and I<liast>. The commands are placed into an editor
for fudging and executed as a shell script. The default is to use the FCEDIT
environment variable as the editor, and to fall back on the EDITOR variable
when it is not set. If the B<-e> option is specified, edit the commands using
I<editor>. If the B<-r> option is specified, the order of commands is inverted.
When the B<-l> option is given, list but do not execute the commands. The B<-n>
option may be used to in conjunction with B<-l> to disable the display of line
numbers; when editing commands, line numbers are never displayed.

The history index range, I<first> and I<last> may be either integer offsets
into the history list, or string patterns.  Relative offsets are possible,
negative indexes count backwards from the current command, thus -1 refers to
the previous command and +1 to the current. When string indexes are used, the
shell searches backwards in history until it finds a line starting with
I<first>, and then walks forward until the previous command or a line starting
with I<last> is found. 
...
may not be convenient at any given moment. As strings, they are evaluated in
the following context: C<m/^first/> and C<m/^last/>.
...
I/O redirection probably only works with simple commands. Been to long since I tested it.

There is no real solid concept of a "Return value" from a command just yet.
This means that conditonal expressions and the like are partially broken. I<if
bad-command; then echo yes; else echo no; fi>  outputs the following:

	tpsh: command not found: bad-command
	yes

Instead of I<no>. Support for this is too experimental to be that far along,
and emphasis was on the code generator being a viable method, rather than on
usability. Like I've always said this is focused on being an interactive shell,
not a portable sh script runner. A proper solution is probably to make command execution contexts return a hash like:
...
Function style macro definitions and positional parameters are not implemented yet.
