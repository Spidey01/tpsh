...
while/until loops are implemented but they are pretty limited because variables
will get expanded BEFORE any code generation occurs. Thus exit coniditons are
limited to a very simple break, continue, next, or last keyword.
...
Running commands via C<tpsh -c 'cmd'> is broken. The option parser shallows
I<'cmd'> :/.
