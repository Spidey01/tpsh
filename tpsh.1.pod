...
Next tpsh examines if the command is built-in to the shell. If so, the
command is executed internally without spawning a new process. tpsh provides
many built-in commands either for connivance or portability; they are
documented under L</Built-in Commands>.
...
The default search path for executables. PATH is a HASHSEP separated list of
